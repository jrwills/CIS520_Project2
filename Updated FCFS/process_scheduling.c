...
bool shortest_remaining_time_first(dyn_array_t *ready_queue, ScheduleResult_t *result) 
{
    ...
    // initialze pcb we will be working with to a pcb with the lowest arrival time
    //tempPcb = dyn_array_at(ready_queue, 0);
    
    /*
    
    long totalRunTime = 0;
    int minBurstTime = tempPcb->remaining_burst_time;
    int minBurstIndex = 0;
    
    // run until all processes have completed
    while(!dyn_array_empty(ready_queue)){
        tempPcb = dyn_array_at(ready_queue, 0);                                 // initialize the min burst time to the burst time of the process with the lowest arrival time
        minBurstTime = tempPcb->remaining_burst_time;
        for(int i = 0; i<numProcesses; i++){
            tempPcb = dyn_array_at(ready_queue, i);
            if(tempPcb->arrival <= totalRunTime){                               // only use process that have arrived (arrival time is less than total run time)
                int burstTime = tempPcb->remaining_burst_time;
                if(burstTime < minBurstTime){                                   // find the process with the lowest burst time
                    minBurstTime = burstTime;
                    minBurstIndex = i;
                }
            }
        ...
        tempPcb = dyn_array_at(ready_queue, minBurstIndex);                     // decrement the burst time of the process with the lowest burst time
        virtual_cpu(tempPcb); 
        if(tempPcb->remaining_burst_time <= 0){
            dyn_array_erase(ready_queue, minBurstIndex);                        // remove a process once it has finished running
            numProcesses--;                                                     // decrement the number of processes
        }
        
        totalRunTime++;
        //numProcesses = dyn_array_size(ready_queue);
    }*/
